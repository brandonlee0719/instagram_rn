...
import {
  ...
  addStoryReaction as addStoryReactionAPI,
...
export const useStories = () => {
      ...
      const groupOfStories = {
        ...
        items: rawStory.map(item => {
          return {
            ...
            myReaction: item?.reactions?.includes(currentUser?.id),
          }
        }),
      }
  ...
  const addStoryReaction = async (storyID, userID) => {
    await addStoryReactionAPI(storyID, userID)
  }
