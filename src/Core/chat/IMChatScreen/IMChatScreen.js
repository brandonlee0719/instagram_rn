...
const IMChatScreen = props => {
  ...
  const onSendInput = async () => {
    ...
    const participantProfilePictureURLs = getParticipantPictures(channel)
  ...
  const getParticipantPictures = channel => {
  ...
  const startUpload = uploadData => {
    ...
    storageAPI.processAndUploadMediaFileWithProgressTracking(
      ...
      async (url, thumbnailURL, urlKey = '', thumbnailKey = '') => {
        if (url) {
          setDownloadObject({
            ...uploadData,
            source: url,
  ...
  const onForwardMessageActionPress = useCallback(async (channel, message) => {
    ...
    const participantProfilePictureURLs = getParticipantPictures(hydrateChannel)
