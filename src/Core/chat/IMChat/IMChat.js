...
function IMChat(props) {
  ...
  const [didCalHeight, setDidCalHeight] = useState(false)
  ...
  useEffect(() => {
    setTimeout(() => {
      // We do this because it takes a few sec for KeyboardAwareView to calculate it's
      // child component height on layout. And this distort it's child contents
      setDidCalHeight(true)
    }, 500)
    return () => {
      handleIsUserTyping('')
    }
  ...
  return (
    <SafeAreaView style={styles.personalChatContainer}>
      <KeyboardAwareView
        ...
        innerViewStyle={styles.nonkeyboardContainer}>
        {didCalHeight && (
          <>
            <MessageThread
              messages={messages}
